...
$remote_xlsx_url = "https://cdn.alko.fi/product-catalog.csv"; // Alkon reaaliaikainen URL
$local_xlsx_file = "alko_products.csv";  // tiedosto levyyn
...
function updatePriceListIfNecessary($remote, $local) {
    $remoteHeaders = get_headers($remote, 1);
    if (!isset($remoteHeaders['Last-Modified'])) return;

    $remoteDate = strtotime($remoteHeaders['Last-Modified']);
    $localDate = file_exists($local) ? filemtime($local) : 0;

    // jos remote on uudempi → lataa
    if ($remoteDate > $localDate) {
        file_put_contents($local, file_get_contents($remote));
        touch($local, $remoteDate);
    }
}
